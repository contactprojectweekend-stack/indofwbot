...
async function seedAdmin() {
    try {
        await db_1.pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_vip BOOLEAN DEFAULT FALSE');
        await db_1.pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_dummy BOOLEAN DEFAULT FALSE');
        const email = 'contact.projectweekend@gmail.com';
        const defaultPass = 'Indonesia';
        const check = await db_1.pool.query('SELECT id FROM admins WHERE email = $1', [email]);
        if (check.rowCount === 0) {
            const hash = await bcrypt_1.default.hash(defaultPass, 12);
            await db_1.pool.query(`INSERT INTO admins (email, password_hash, role) VALUES ($1, $2, 'SUPER_ADMIN')`, [email, hash]);
...
async function recordAudit(adminId, action, targetType, targetId, ip, meta) {
    try {
        await db_1.pool.query(`INSERT INTO admin_audit_logs (admin_id, action, target_type, target_id, ip_address, metadata) VALUES ($1, $2, $3, $4, $5, $6)`, [adminId, action, targetType, targetId, ip, JSON.stringify(meta)]);
    }
    catch (e) {
...
function requireAdmin(req, res, next) {
    ...
    try {
        req.admin = jsonwebtoken_1.default.verify(token, JWT_SECRET);
        next();
    }
    catch {
...
router.post('/login', async (req, res) => {
    ...
    const ip = req.ip || '';
    const result = await db_1.pool.query('SELECT * FROM admins WHERE email = $1 AND is_active = TRUE', [email]);
...
router.get('/users', requireAdmin, async (req, res) => {
    const data = await db_1.pool.query(`
    SELECT u.*, 
...
// 3. TOGGLE VIP STATUS
router.post('/users/:id/toggle-vip', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const userRes = await db_1.pool.query('SELECT is_vip, display_name FROM users WHERE id = $1', [id]);
        if (userRes.rowCount === 0)
            return res.status(404).json({ error: 'Pengguna tidak ditemukan' });
        const newVipStatus = !userRes.rows[0].is_vip;
        await db_1.pool.query('UPDATE users SET is_vip = $1 WHERE id = $2', [newVipStatus, id]);
...
router.get('/grouped-conversations', requireAdmin, async (req, res) => {
    try {
        const query = `
      SELECT 
...
router.delete('/conversations/room/:connectionId', requireAdmin, async (req, res) => {
    try {
        const { connectionId } = req.params;
        await db_1.pool.query('DELETE FROM messages WHERE connection_id = $1', [connectionId]);
        await db_1.pool.query('DELETE FROM connections WHERE id = $1', [connectionId]);
...
router.delete('/conversations/clear-all', requireAdmin, async (req, res) => {
    try {
        await db_1.pool.query('DELETE FROM messages');
        await db_1.pool.query('DELETE FROM connections');
...
router.get('/photo-view/:id', async (req, res) => {
    try {
        const photo = await db_1.pool.query('SELECT storage_path, file_url FROM user_photos WHERE id = $1', [req.params.id]);
        if (photo.rowCount === 0)
        ...
        try {
            const freshLink = await bot_1.bot.telegram.getFileLink(storage_path);
...
router.get('/media-proxy', async (req, res) => {
    try {
        ...
        if (!fileId)
            return res.status(400).send('No file_id');
        if (fileId.startsWith('http'))
            return res.redirect(fileId);
        const freshLink = await bot_1.bot.telegram.getFileLink(fileId);
        return res.redirect(freshLink.href);
    }
    catch {
...
router.post('/create-dummy-user', requireAdmin, async (req, res) => {
    try {
        ...
        const photoList = Array.isArray(photos) ? photos.map((p) => p.trim()).filter((p) => p.length > 0) : [];
        let finalPrefs = Array.isArray(gender_preferences) ? gender_preferences : [gender_preferences];
        ...
        const userRes = await db_1.pool.query(`INSERT INTO users (telegram_id, username, display_name, age, gender, gender_preferences, relationship_goals, profile_completed, is_active, is_vip, is_dummy)
       VALUES ($1, $2, $3, $4, $5::gender_type, $6::gender_type[], $7::goal_type[], TRUE, TRUE, $8, TRUE)
       RETURNING id`, [fakeTgId, cleanUsername, display_name, ageNum, gender, finalPrefs, relationship_goals, is_vip === true]);
        const newUserId = userRes.rows[0].id;
        for (let i = 0; i < photoList.length; i++) {
            await db_1.pool.query(`INSERT INTO user_photos (user_id, storage_path, file_url, is_primary) VALUES ($1, $2, $3, $4)`, [newUserId, photoList[i], photoList[i], i === 0]);
        }
...
router.post('/clean-dummy-users', requireAdmin, async (req, res) => {
    try {
        await db_1.pool.query(`DELETE FROM users WHERE is_dummy = TRUE OR telegram_id >= 999000000`);
...
router.get('/', (req, res) => {
    const htmlPath = path_1.default.resolve(process.cwd(), 'src', 'views', 'admin.html');
